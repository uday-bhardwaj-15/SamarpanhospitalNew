...
const Navbar = () => {
  ...
  const navLinks = [
    { name: "Home", href: "/" },
    { name: "Our services ", href: "/departments" },
    { name: "Team", href: "/team" },
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
        showScrolledStyle ? "glass py-3 shadow-soft" : "bg-transparent py-6"
      }`}
    >
      <div className="container-custom flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            <span
              className={`text-xl font-bold transition-colors duration-300 ${showScrolledStyle ? "text-foreground" : "text-white"}`}
            >
              Samarpan Hospital
            </span>
            <span
              className={`text-xs font-medium transition-colors duration-300 ${showScrolledStyle ? "text-muted-foreground" : "text-white/70"}`}
            >
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <Button
            variant={showScrolledStyle ? "hero" : "hero-light"}
            size="default"
        ...
        >
          {isMobileMenuOpen ? (
            <X
              className={`w-6 h-6 ${showScrolledStyle ? "text-foreground" : "text-white"}`}
            />
          ) : (
            <Menu
              className={`w-6 h-6 ${showScrolledStyle ? "text-foreground" : "text-white"}`}
